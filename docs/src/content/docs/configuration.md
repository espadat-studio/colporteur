...
| `base_url`    | string  | _optional_ | Base URL for serving feeds; used to generate `<link rel="alternate">` entries. Must be `http://` or `https://`, no `?` or `#`. Trailing slash is trimmed |
...
| `url`         | string   | _optional_           | URL added as `<link rel="alternate">` on each entry; overrides `base_url`. Must be `http://` or `https://` |
